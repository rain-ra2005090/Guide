...
const checkScriptMatch = async (question) => {
  return new Promise((resolve, reject) => {
    execFile("python", ["script_matcher.py", question], (error, stdout, stderr) => {
      if (error) {
        console.error("❌ Script Matcher Error:", stderr);
      ...
      try {
        ...
        console.error("❌ Failed to parse script matcher output:", stdout);
        resolve({ match: false });
      }
    });
  });
...
app.post("/chat", async (req, res) => {
  ...
  const character = req.body.character || "lolwa"; // default to rami if none
  ...
  try {    let responseText = "";
    const scriptMatch = await checkScriptMatch(userMessage);

    if (scriptMatch.match) {
      console.log("🎯 Script Match Found:", scriptMatch.question);
      responseText = scriptMatch.answer;
    } else {
      const systemInstruction = SYSTEM_INSTRUCTIONS[character.toLowerCase()] || SYSTEM_INSTRUCTIONS.lolwa;
      responseText = await generateAIResponse(userMessage, systemInstruction);
    }
